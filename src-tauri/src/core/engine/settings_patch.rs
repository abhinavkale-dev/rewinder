...
impl Engine {
        ...
        *self.mic_device_not_found_warned.lock() = false;
        ...
        if let Err(err) = self.ensure_pipeline_for_state() {
            ...
            let full_err: String = match rollback_restart_err {
                Some(restart_err) => {
                    format!("{source}: apply failed: {err}; rollback restart failed: {restart_err}")
                }
                None => format!("{source}: apply failed: {err}"),
            };
    ...
    pub fn resume_capture(&self, app: &AppHandle) -> Result<EngineStateDto, String> {
